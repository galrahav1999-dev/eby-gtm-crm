    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-x-8 md:grid-cols-3">
        <DetailField label="Person">
          {it.people ? <Link href={`/people/${it.people.id}`} className="text-primary hover:underline">{personName(it.people)}</Link> : null}
        </DetailField>
        <DetailField label="Organization">
          {it.organizations ? <Link href={`/organizations/${it.organizations.id}`} className="text-primary hover:underline">{it.organizations.name}</Link> : null}
        </DetailField>
        <DetailField label="Deal">
          {it.deals ? <Link href={`/deals/${it.deals.id}`} className="text-primary hover:underline">{it.deals.name}</Link> : null}
        </DetailField>
      </div>

      <DetailField label="What happened / outcome">{it.outcome}</DetailField>
      <DetailField label="Verbatim quote / key signal">
        {it.verbatim_quote && <span className="italic text-ink-soft">“{it.verbatim_quote}”</span>}
      </DetailField>
